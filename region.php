<?php
  ...
  if (!empty($_GET["sights_name"])){
    ...
    switch ($mode_id){
        case 0:
        $sql = "SELECT sights.sights_id, sights.sights_name,sights.sights_intro, photo.photo_file, count(photo.sights_id)";
   ...
   if (!empty($_GET["region_id"])){
    ...
    $id=$_GET["region_id"];
    $sql = "SELECT sights.sights_id, sights.sights_name,sights.sights_intro, photo.photo_file, count(photo.sights_id)";
    $sql .= " FROM sights, photo,region";
    $sql .= " where sights.sights_region_id='" .$id. "' and region.region_id=sights.sights_region_id and sights.sights_id=photo.sights_id";
    $sql .= " GROUP BY sights.sights_id";
    $result = mysqli_query($con,$sql);
    }
...
<body>
    ...
    <main>
        <div class="container2">
            <h1><?php if (isset($name)) echo $name; else echo "景點總覽"; ?></h1>
            <label for="name"> 景點總數：<?php echo $total_records;?></label><p> 
            <section class='cards'>
            <?php //顯示記錄
            $j = 1;
            while($row=mysqli_fetch_assoc($result) and $j<=$records_per_page) {
                switch ($j)
                 ...
                 //資料庫-介紹並縮減內容
                    echo mb_substr($row['sights_intro'],0, 35,'UTF-8')."<br>";
                    ...
                    <ul class="pagination">
                        <li class="page-item">
                            ...
                            if($page>1)
                                ...
                                {   
                                    $id = $_GET["region_id"];
                                    echo "<li class='page-item'><a class='page-link' href='region.php?region_id=$id&page=".($page-1)."'>上一頁</a> </li> ";
                                    for($i=1; $i<=$total_pages; $i++) {
                                        if($i==$page)
                                        echo "<li class='page-item'><a class='page-link' >$i</a></li> ";
                                        else
                                        echo "<li class='page-item'><a class='page-link' href='region.php?region_id=$id&page=$i'>$i</a></li> ";
                                    }
                                    if($page<$total_pages)
                                        echo "<li class='page-item'><a class='page-link' href='region.php?region_id=$id&page=".($page+1)."'>下一頁</a></li>";
                                    echo "</p>";
                                ...
                                if(!empty($_GET["sights_name"]))
                                {
                                    ...
                                    switch ($mode_id){
                                        case 0:
                                            echo "<li class='page-item'><a class='page-link' href='region.php?sights_name=$name&searchmode=$mode_id&page=".($page-1)."'>上一頁</a> </li> ";
                                            for($i=1; $i<=$total_pages; $i++) {
                                                if($i==$page)
                                                echo "<li class='page-item'><a class='page-link' >$i</a></li> ";
                                                else
                                                echo "<li class='page-item'><a class='page-link' href='region.php?sights_name=$name&searchmode=$mode_id&page=$i'>$i</a></li> ";
                                            }
                                            if($page<$total_pages)
                                                echo "<li class='page-item'><a class='page-link' href='region.php?sights_name=$name&searchmode=$mode_id&page=".($page+1)."'>下一頁</a></li>";
                                            echo "</p>";
                                        break;
                                        case 1:
                                            echo "<li class='page-item'><a class='page-link' href='region.php?sights_name=$name&searchmode=$mode_id&page=".($page-1)."'>上一頁</a> </li> ";
