...
if (isset($_GET['delete'])) {
    ...
    $delsql = "DELETE FROM`share` WHERE `itinerary_id` = " . $del . " and user_id='" . $user_id . "'";
    if (mysqli_query($con, $delsql)) {
        header("Location: shareitinerary.php");
    } else {
        echo "<script> alert('移除失敗');parent.location.href='shareitinerary.php'; </script>";
    }
}
//顯示景點
$sql = "SELECT itinerary.itinerary_id, itinerary.itinerary_name, itinerary.public_status, itinerary.itinerary_date, itinerary.itinerary_days , itinerary.user_id,user.user_name";
$sql .= " FROM itinerary,share,user WHERE share.user_id = '$user_id' ";
$sql .= "and share.itinerary_id=itinerary.itinerary_id and itinerary.user_id= user.user_id ";
        ...
        <div class="container2">
            ...
            <div id="left">
                ...
                while ($row = mysqli_fetch_assoc($result)  and $j <= $records_per_page) {;
                    ...
                    echo "<div >";
                    echo "<h4><a href='modifyitinerary.php?id=" . $itinerary_id . "&share=1' style='text-decoration:none; color:black;'>" . $row["itinerary_name"] . "</a>";
            ...
            <td colspan="2" align="center" valign="top">
                <ul class="pagination">
                    <li class="page-item">
                        ...
                        if ($total_pages > 1)
                            if ($page > 1) {
                                echo "<li class='page-item'><a class='page-link' href='shareitinerary.php?page=" . ($page - 1) . "'>上一頁</a> </li> ";
                                for ($i = ($page - 2); $i <= min($total_pages, $page - 1); $i++) {
                                    if ($i == $page)
                                        ...
                                        echo "<li class='page-item'><a class='page-link' href='shareitinerary.php?page=$i'>$i</a></li> ";
                                }
                            }
                        ...
                        if ($page < $total_pages) {
                            echo "<li class='page-item'><a class='page-link' href='shareitinerary.php?page=" . ($page + 1) . "'>下一頁</a></li>";
                            echo "</p>";
                        }
